    ...
    return userService.getByEmail(email)
        .flatMap(user -> {
          if (password.equals(user.getPassword())) {
            return Optional
                .of(new JwtAuthenticationToken(email, password, Collections.singletonList(new SimpleGrantedAuthority("ROLE_" + user.getRole().name())), JwtUtil.generateToken(user, secret)));
          }
